...
use database::access_control::{
  select_group_grant_perm_stream, select_group_membership_stream, select_object_grant_perm_stream,
  AFGroupGrantPermRow, AFGroupMembershipRow, AFObjectGrantPermRow,
};
...
fn access_level_act(level: i32) -> Option<String> {
  ...
  Some(level.to_enforce_act())
}

/// The casbin subject string for a group, namespaced with `g:` to avoid
/// collision with numeric user ids. Must match the biz layer's group subject.
pub fn group_subject(group_id: &uuid::Uuid) -> String {
  format!("g:{}", group_id)
}
...
pub async fn load_object_grant_policies(
  ...
  while let Some(Ok(grant)) = stream.next().await {
    let Some(act) = access_level_act(grant.access_level) else {
      continue;
    };
...
/// Loads group object-level grants as casbin policies with a group subject:
/// `["g:<group_id>", "collab::<object_id>", "l:<level>"]`. Members resolve to
/// these via the `g2` grouping.
pub async fn load_group_grant_policies(
  mut stream: BoxStream<'_, sqlx::Result<AFGroupGrantPermRow>>,
) -> Result<Vec<Vec<String>>> {
  let mut policies: Vec<Vec<String>> = Vec::new();
...
/// Loads user -> group memberships as casbin `g2` grouping rules
/// `[uid, "g:<group_id>"]`.
pub async fn load_group_membership_rules(
  mut stream: BoxStream<'_, sqlx::Result<AFGroupMembershipRow>>,
) -> Result<Vec<Vec<String>>> {
  let mut rules: Vec<Vec<String>> = Vec::new();

  while let Some(Ok(membership)) = stream.next().await {
    rules.push(vec![
      membership.uid.to_string(),
      group_subject(&membership.group_id),
    ]);
  }
...
impl Adapter for PgAdapter {
  async fn load_policy(&mut self, model: &mut dyn Model) -> Result<()> {
    ...
    // Object-level group grants (per-collab access levels for a group subject).
    let group_grant_stream = select_group_grant_perm_stream(&self.pg_pool);
    let group_grant_policies = load_group_grant_policies(group_grant_stream).await?;
    model.add_policies("p", "p", group_grant_policies);

    // User -> group memberships as the `g2` grouping.
    let group_membership_stream = select_group_membership_stream(&self.pg_pool);
    let group_membership_rules = load_group_membership_rules(group_membership_stream).await?;
    model.add_policies("g", "g2", group_membership_rules);
